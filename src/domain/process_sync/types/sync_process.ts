...
export const isProcessSync = (data: unknown): data is ProcessSync => {
	...
	if (error) {
		log(` Data is not process sync - ${data} - ${error} process_sync.ts`, LogLevel.ERROR);
	}
	return success;
};

export const processSyncInsertSchema = processSyncSchema.omit({ id: true, createdAt: true, updatedAt: true });
